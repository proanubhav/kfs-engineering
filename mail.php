...
$phone = isset($_POST['phone']) ? $_POST['phone'] : 'Madover Finance';
...
try {
    ...
    $mail->setFrom('proanubhav@gmail.com', 'Mad over Finance Enquiry');
    ...
    $mail->Body.='<p style="color: #fff; text-align: center; font-size: 45px;font-weight:600;margin:0">Welcome to <span style="font-size:55px">Madover Finance</span></p>';
    $mail->Body.='</div>';
    ...
    $mail->Body.='<p style="font-size: 16px;"><strong>Hello </strong>Madover Finance,</p>';
    $mail->Body.='<p style="font-size: 16px;">Someone has tried to contact Madover Finance team.</p>';
    ...
    $mail->Body.='<p style="font-size: 16px;">Madover Finance</p>';
    $mail->Body.='</div>';
    $mail->Body.='</div>';
    $mail->Body.='</body>';
    $mail->Body.='</html>';
    $mail->Subject = 'Madover Finance';
    $mail->send();
    echo '1';
} catch (Exception $e) {
